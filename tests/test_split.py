...
class TestRowLevelSplit:
    ...
    def test_no_overlap(self):
        ...
        valid_descs = {e.description for e in split.valid}
        test_descs = {e.description for e in split.test}

        assert len(train_descs & valid_descs) == 0
        assert len(train_descs & test_descs) == 0
        assert len(valid_descs & test_descs) == 0
...
class TestRulingLevelSplit:
    def test_ruling_isolation(self):
        ...
        valid_rulings = {e.ruling_number for e in split.valid if e.ruling_number}
        test_rulings = {e.ruling_number for e in split.test if e.ruling_number}

        assert len(train_rulings & valid_rulings) == 0
        assert len(train_rulings & test_rulings) == 0
        assert len(valid_rulings & test_rulings) == 0
    ...
    def test_no_ruling_examples_go_to_train(self):
        ...
        valid_no_ruling = [e for e in split.valid if e.ruling_number is None]
        test_no_ruling = [e for e in split.test if e.ruling_number is None]
        assert len(valid_no_ruling) == 0
    ...
    def test_deterministic(self):
        ...
        rulings1 = sorted(e.ruling_number for e in split1.valid if e.ruling_number)
        rulings2 = sorted(e.ruling_number for e in split2.valid if e.ruling_number)
...
class TestLeakageDetection:
    ...
    def test_detects_exact_duplicate_leak(self):
        ...
        leaks = detect_leakage(train, valid, [], threshold=0.5)
        assert leaks.get("valid_vs_train", 0) > 0
