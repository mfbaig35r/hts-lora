# Parameter-Efficient HTS Classification at the Edge: An 8B LoRA Adapter Matches a 70B Benchmark for $43 of Training Compute
...
We present v1 of an open-weight LoRA adapter for U.S. Harmonized Tariff Schedule (HTS) classification, trained on Llama-3.1-Nemotron-Nano-8B using 119,602 CROSS ruling examples on a single H100 GPU for approximately $43 of training compute. The model produces structured chapter-heading-subheading-code output suitable for direct consumption by downstream pipelines. On the public 200-example ATLAS test set (Yuvraj & Devarakonda 2025), our model achieves 41.0% exact-match accuracy (HF transformers + bitsandbytes nf4 4-bit inference), statistically indistinguishable from the ATLAS paper's reported Llama-3.3-70B full SFT result of 40.0% at 1/8 the parameter count. The base Nemotron-Nano-8B model with no adapter achieves 0.0% on the same test, demonstrating that the LoRA contributes essentially all the task capability. On a held-out CROSS-derived test set of 14,952 examples, the model achieves 58.7% exact-match (95% CI ±0.8pp) and 92.0% chapter accuracy. Production deployment via MLX q4 quantization on M4 Mac mini hardware achieves 36.0% ATLAS exact-match with 4.7 second single-stream latency, 11pp above the ATLAS-reported GPT-5-Thinking zero-shot baseline of 25.0%. We additionally report a measured negative ablation: stat-suffix validation against a training-derived index of 9,791 valid 10-digit codes produces no measurable accuracy lift, because the model already emits valid HTSUS codes in 87-96% of predictions. The dominant failure mode is wrong-but-valid prediction rather than invalid hallucination, indicating that future improvements should target disambiguation (retrieval-augmented inference) rather than validation. All code, weights, evaluation data, and reproducibility artifacts are available.
...
The HTS classification task has attracted recent attention as a natural target for language model specialization. The hierarchical structure of the schedule, the availability of CBP's published binding rulings (CROSS) as a training corpus, and the high commercial value of automated classification combine to make it an unusually clean specialization problem. Two published benchmarks anchor the field: Yuvraj & Devarakonda (2025, the "ATLAS paper," arXiv:2509.18400) report 40.0% exact-match accuracy on N=200 with a fully fine-tuned Llama-3.3-70B model and 25.0% with zero-shot GPT-5-Thinking; Judy (2024, arXiv:2412.14179) reports 89.2% on N=103 with the Tarifflo retrieval-augmented pipeline and 80.0% with an Avalara expert-plus-AI workflow.

These results bracket the design space: model-only specialization with sufficient parameter count appears to reach roughly 40% exact-match on benchmark test sets, while retrieval-augmented architectures with smaller test sets reach 80-90%. The architectural choice matters more than the parameter count.

A gap in this design space has not been addressed by published work: whether a substantially smaller model, on the order of 8 billion parameters rather than 70 billion, with parameter-efficient fine-tuning rather than full SFT, can reach the model-only benchmark frontier. If so, the deployment economics change qualitatively. A model that fits in 5 GB at 4-bit quantization runs interactively on consumer hardware (specifically, the 16 GB M4 Mac mini, MSRP $700). The recurring marginal cost of classification drops from "API call to a hosted frontier model" to zero.

This paper reports a v1 result for that question. We trained a LoRA adapter (Hu et al. 2022) on Llama-3.1-Nemotron-Nano-8B using 119,602 CROSS ruling examples on a single H100 GPU for approximately $43 of compute and 14 hours of wall time. The resulting 336 MB adapter, when merged into the base model and deployed via MLX q4 quantization on an M4 Mac mini, achieves 36.0% exact-match accuracy on the ATLAS public test set. The same adapter under HF transformers with bnb-nf4 4-bit inference achieves 41.0%, statistically matching the ATLAS-reported 70B SFT benchmark. We provide all weights, code, training data sources, evaluation harness, and edge deployment recipe.

We additionally report a methodological negative result. The dominant failure mode on both test sets is at the 10-digit statistical suffix: 2,458 of 6,206 errors on our held-out test set (39.5%) and 23 of 116 errors on ATLAS (19.8%) are cases where the model predicts the correct 8-digit subheading but the wrong 2-digit suffix. We constructed the obvious post-processing fix: a training-derived index of valid 10-digit completions per 8-digit subheading, with a most-frequent-completion picker for invalid predictions. The measured lift on both test sets is essentially zero. Investigation reveals that the model already produces valid HTSUS 10-digit codes in 87.3% of held-out predictions and 96.5% of ATLAS predictions. The dominant failure mode is wrong-but-valid prediction rather than invalid hallucination; stat-suffix validation cannot address it. The path to the next factor of accuracy improvement therefore lies in disambiguation (retrieval-augmented inference, code-embedding nearest-neighbor reranking) rather than output validation.

The contributions of this paper are:

1. A working open-weight 8B LoRA HTS classifier that statistically matches the published 70B SFT benchmark on the ATLAS test set at 1/8 the parameter count and approximately $43 of training compute.
...
**Domain-specific HTS classification.** Yuvraj & Devarakonda (2025) introduce the ATLAS benchmark, a 200-example public test set, and report results for several configurations including a fully fine-tuned Llama-3.3-70B model (40.0% exact-match), zero-shot GPT-5-Thinking (25.0%), and zero-shot GPT-4 variants. Their training pipeline uses approximately 18,000 examples derived from CROSS rulings with label-conditioned reasoning trace generation. Judy (2024) introduces the Tarifflo benchmark (103 examples) and reports retrieval-augmented pipeline results at 89.2%, with the Avalara workflow at 80.0%. These two benchmarks bracket the field but are not directly comparable to each other: the test sets are distinct, the prompt formats differ, and the architectural assumptions are inverted (model-only vs pipeline).

**Parameter-efficient fine-tuning.** LoRA (Hu et al. 2022) decomposes the weight update of a fine-tuning step into low-rank matrices, enabling task adaptation with two to three orders of magnitude fewer trainable parameters than full SFT. Our adapter has 32M trainable parameters against an 8B base, a ratio of approximately 0.4%. The training data and architectural choices in this paper are conventional for LoRA fine-tuning of an instruction-tuned base model.

**Structured output for hierarchical classification.** Recent work on schema-constrained generation (Lin et al. 2024, several others) suggests that hierarchical classification problems benefit from explicit hierarchy in the output format. Our v2 output format requires the model to emit Chapter, Heading, Subheading, HTS Code, and Reasoning sections in a fixed order. This is not strictly constrained decoding; the model is free to produce malformed output and the parser flags it. In practice, parse failure occurs at a rate of 0.06% on our held-out test (9 of 14,952), suggesting the model has internalized the format via supervised fine-tuning rather than requiring runtime enforcement.

**Quantization-aware deployment.** The MLX framework (Hannun et al. 2024) provides Apple Silicon-native inference with native q4 and q8 quantization. The bitsandbytes library (Dettmers et al. 2022) provides CUDA-native nf4 quantization. These two quantization schemes are not bit-equivalent: bnb nf4 uses normal-float 4-bit with optional double quantization; MLX q4 uses a simpler affine quantization. The accuracy gap between them, in our measurements, is approximately 5 percentage points on ATLAS exact-match.

**Negative results in NLP.** Reporting failed approaches alongside successful ones is a well-established practice in classical NLP and ML (e.g., Rogers et al. 2020 on probing studies, others). Our stat-suffix validation negative result is reported in Section 6 not as a failure of the experimental design but as positive evidence about the model's actual failure distribution.
...
The base model is `nvidia/Llama-3.1-Nemotron-Nano-8B-v1` (8.1 billion parameters), an instruction-tuned variant of Llama 3.1 8B. We apply a LoRA adapter (Hu et al. 2022) targeting the `q_proj`, `k_proj`, `v_proj`, `o_proj`, `gate_proj`, `up_proj`, and `down_proj` modules across all 32 transformer layers. LoRA hyperparameters: rank 32, alpha 64 (effective scale 2.0), dropout 0.05. The total parameter count is 32M trainable plus 8.1B frozen.

### 3.3 Training procedure

Training was performed on a single rented H100 SXM 80GB GPU on RunPod. Hyperparameters: 3 epochs, effective batch size 32 (per-device batch 16, gradient accumulation 2), max sequence length 1536, learning rate 2.0e-4 with cosine schedule and 5% warmup, AdamW optimizer in paged 8-bit configuration, bfloat16 mixed precision, 4-bit nf4 quantization of the base model via bitsandbytes for memory efficiency. Attention implementation: scaled dot-product attention (SDPA); we did not use Flash Attention 2 due to ABI compatibility issues with the available PyTorch build (documented at length in `docs/log-2026-04-06-h100-training.md`).
...
The training run kept the last 3 checkpoints by default (`save_total_limit: 3`). The actually-best checkpoint by eval loss occurred at step 7000 (eval loss 0.2596, epoch 1.87), but was overwritten by later checkpoints with higher eval loss. The shipped adapter is the final-step checkpoint (step 11214, eval loss 0.2729). The accuracy numbers reported in this paper are for the shipped final-step adapter, not the lost best checkpoint. We estimate the lost checkpoint would have produced approximately 1-3pp higher exact-match accuracy based on the typical eval-loss-to-accuracy correspondence for this task; we have not recovered it.

### 3.4 Evaluation

We evaluate on two test sets.

**ATLAS public test (N=200).** The 200-example test set published by Yuvraj & Devarakonda (2025). We use their original test file and convert it to our v2 messages format via `scripts/build_atlas_eval.py`. The ATLAS test set has 31 multi-code rows (15.5%) where multiple correct codes are accepted; we score using any-of-set match semantics to match the ATLAS paper's scoring.
...
We measured three inference configurations to isolate the accuracy contribution of the inference backend versus the quantization scheme:
...
Published comparators on the same test set:

| System | Parameters | Exact match | Source |
|---|---|---|---|
| ATLAS Llama-3.3-70B SFT | 70B (full SFT) | 40.0% | Yuvraj & Devarakonda 2025 |
| GPT-5-Thinking zero-shot | (closed) | 25.0% | Yuvraj & Devarakonda 2025 |

Our v1 LoRA at HF nf4 inference (41.0%) is statistically indistinguishable from the ATLAS 70B SFT baseline (40.0%); the 95% CI on each spans the other. The base model alone is essentially incapable of producing structured HTS classifications in our v2 format (parse rate 12.5%, exact-match 0.0%), demonstrating that the LoRA fine-tune contributes essentially all the task capability.
...
The MLX bf16 diagnostic result (42.0% on N=50) confirms that our HF-to-MLX adapter conversion is correct: the unquantized MLX inference path produces accuracy statistically equivalent to the HF nf4 path. The 5pp gap between HF nf4 (41.0%) and MLX q4 (36.0%) is therefore attributable to quantization scheme differences, not to inference-path or conversion artifacts.
...
We constructed and measured a stat-suffix validation post-processor with the goal of addressing the right_subheading_wrong_full error bucket. The procedure:
...
1. **Stat-suffix validation against a more complete index (e.g., the official USITC HTSUS export covering all ~19,000 codes) would produce marginal additional lift, not transformative lift.** The bottleneck is not coverage of the validation index; it's the fact that the model rarely produces invalid codes in the first place.

2. **Constrained decoding (forcing the model to emit only codes from a known valid set) would also produce marginal lift.** Same reason.

3. **Disambiguation between valid candidate codes is the open architectural lever.** Retrieval-augmented inference, in which top-K similar CROSS rulings or HTSUS schedule entries are injected into the model's context before generation, is the natural next architecture. The Tarifflo benchmark result (89.2% with a retrieval-augmented pipeline) suggests substantial available headroom from this direction.
...
For comparison, the ATLAS paper's 70B SFT training used 16 A100s; the published cost has not been disclosed but is likely several thousand dollars per training run, with ongoing inference costs requiring continued GPU access.
...
**Training data distribution.** The training corpus is drawn entirely from CBP CROSS rulings. CROSS represents binding rulings, which skew toward edge cases, novel products, and contested classifications. The distribution of typical importer queries is different (more high-volume standard products, less interpretive ambiguity). The 58.7% accuracy on our held-out test set should not be read as the accuracy on real-world traffic.

**Test set size for the public benchmark.** The ATLAS test set is N=200, giving the headline 41.0% number a 95% CI of approximately ±7pp. Our held-out test gives a much tighter CI (±0.8pp at N=14,952) but is in-distribution with training data; we cannot use it for cross-system comparison. Tighter public benchmarks for this task remain a community need.

**Quantization configuration matters.** The 41.0% number is for HF transformers + nf4. The 36.0% number is for MLX q4. Both are real. Papers, blog posts, and reproduction attempts should be specific about which configuration is being cited.

**Lost best checkpoint.** The shipped adapter is the final-step checkpoint, not the lowest-eval-loss checkpoint. The actually-best checkpoint (step 7000, eval loss 0.2596) was overwritten by the trainer's default save-total-limit behavior. We estimate this costs 1-3pp on exact-match accuracy. A retrain with `save_total_limit ≥ 5` and `load_best_model_at_end` would recover this; we have not done so.

**Single-stream latency.** The 4.7-second latency figure is single-stream. Throughput under concurrent load is not benchmarked. Neither the MLX server nor our FastAPI wrapper does request batching. For an internal tool this is fine; for a public API it isn't.

**Training-derived stat-suffix index is incomplete.** The 9,791 codes in our validation index are only those that appeared as gold answers in training. The full HTSUS schedule contains approximately 19,000 codes. For production deployment, the index should be replaced with the official USITC export; a follow-up note at `docs/layer-a-followup-usitc.md` documents the procedure. The Section 6 negative-result conclusion does not depend on this completeness, however, because the bottleneck is the model's valid-code emission rate, not the index coverage.
...
First, the implicit assumption in the ATLAS paper and adjacent work that parameter count is the primary axis for model-only HTS classification appears not to be supported. An 8B LoRA on a well-chosen base model with appropriate training data and a structured output format reaches the same benchmark frontier as a 70B full SFT model. The cost asymmetry between these approaches is roughly an order of magnitude in training compute and a similar order in deployment cost.

Second, the implicit assumption in retrieval-augmented pipeline work (Tarifflo) that the model is mostly a "consumer of retrieved context" rather than a primary contributor to accuracy may be incomplete. Our results show that a small model can do most of the heavy lifting on this task without any retrieval at all. The retrieval-augmented architecture is presumably still better at the limit, but the marginal contribution of the model itself in those architectures has not been carefully measured against a strong model-only baseline.

Third, the practical implications for enterprise deployment are larger than the accuracy numbers alone suggest. A specialized model that runs on a $700 commodity device with no recurring cost, no external API dependency, and no data exfiltration risk has a fundamentally different unit economics than any current SaaS or API-based HTS classification offering. The minimum viable product threshold for a customs-broker internal tool, an in-house compliance team's first-pass classifier, or an importer-side review workflow has dropped substantially.

### 9.2 Path to the next factor of improvement
...
The natural next architectural step is retrieval-augmented inference. The specific design we plan for v2 is to inject top-5 most-similar CROSS rulings and the relevant HTSUS chapter notes into the model's context at inference time, with training-time exposure to the same retrieval-augmented prompts so the model learns to ground its reasoning in retrieved evidence. The Tarifflo published result (89.2% on N=103 with full retrieval-augmented pipeline) suggests substantial headroom from this direction.
...
Hannun, A., Digani, J., Katharopoulos, A., & Collobert, R. (2024). MLX: Efficient and Flexible Machine Learning on Apple Silicon. https://github.com/ml-explore/mlx
...
Judy, J. (2024). Tarifflo: A Hybrid Retrieval-and-ML Pipeline for HTS Classification. arXiv:2412.14179.

Lin, B. et al. (2024). [Schema-constrained generation citation; verify before submission.]
...
Yuvraj, R., & Devarakonda, A. (2025). ATLAS: A Benchmark and Fine-Tuning Approach for U.S. Harmonized Tariff Schedule Classification. arXiv:2509.18400.

---

**Note for the author preparing the arXiv submission.** Several citations above are placeholders requiring verification before submission (Lin et al. 2024 on schema-constrained generation is one example). The Hannun et al. MLX citation is to the GitHub repository because a formal paper is not available; substitute the canonical citation if MLX has been published since this draft. The Tarifflo and ATLAS papers cited here have been used as field anchors throughout this work; cross-reference the arXiv IDs and author names against the published versions before submission. The "released openly" claim in Section 9.3 presumes the GitHub repository and HuggingFace adapter are toggled to public at submission time.
